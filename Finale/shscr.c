...
void print_help(){
	...
	printf("~remove <file name> - removes a file of the specified name\n");
...
int main_shell_loop(){
	///////////////// EXIT ////////////////////////////////////
		...
		//////////////////// SCRIPT ////////////////////////
				...
				if(args[1]==NULL) printf("%sError:%s No script file name specified!", C_RED, C_NRM);
				else{
					if(access(args[1],F_OK) != -1) operation_type=1;
					else printf("%sError:%s Specified script does not exist!", C_RED, C_NRM);
				}
			}
		/////////////////// END SCRIPT //////////////////
...
int main()
{
	while(1)	
	{
		...
		if(operation_type==0){
			...
			FILE * s_source;
			s_source = fopen(args[1], "r");
			while(fgets(cmd_line_input, sizeof(cmd_line_input), s_source)){
				main_shell_loop();
			}
			fclose(s_source);
			operation_type=0;
